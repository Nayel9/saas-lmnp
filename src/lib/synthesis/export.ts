...
export function buildPdfTextModel(opts: {
  propertyLabel: string; year: number;
  ...
  L.push('Bilan (simple)');
  L.push('Actif:');
  L.push(`- Immobilisations (valeur restante): ${formatEUR(balance.actif.vnc)}`);
  L.push(`- Trésorerie (année): ${formatEUR(balance.actif.treso)}`);
  if (balance.totals) L.push(`= Total Actif: ${formatEUR(balance.totals.actif)}`);
  L.push('Passif:');
  L.push(`- Cautions détenues: ${formatEUR(balance.passif.cautions)}`);
  L.push(`- Autres dettes: ${formatEUR(balance.passif.dettes)}`);
  if (balance.totals) L.push(`= Total Passif: ${formatEUR(balance.totals.passif)}`);
