...
export async function createAttachmentForUser(userId: string, data: { entryId?: string; assetId?: string; fileName: string; fileSize: number; mimeType: string; storageKey: string; }) {
...
export async function listAttachmentsForUser(userId: string, parent: { entryId?: string; assetId?: string }) {
  ...
  if (entryId) {
    ...
    if (!entry) throw new Error('FORBIDDEN');
    return prisma.attachment.findMany({ where: { entryId }, orderBy: { createdAt: 'desc' } });
  }
  const asset = await ensureAssetOwned(userId, assetId!);
  if (!asset) throw new Error('FORBIDDEN');
  return prisma.attachment.findMany({ where: { assetId }, orderBy: { createdAt: 'desc' } });
}

export async function deleteAttachmentForUser(userId: string, attachmentId: string) {
  const att = await prisma.attachment.findUnique({ where: { id: attachmentId }, include: { entry: true, asset: true } });
  ...
  try { await deleteObject(att.storageKey); } catch { /* ignore */ }
