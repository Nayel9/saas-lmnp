...
beforeAll(async () => {
  ...
  const u = await prisma.user.create({
    data: { email, password: hash, role: "user", emailVerified: new Date() },
  });
  userId = u.id;
...
describe("attachments service", () => {
  it("creates attachment for owned entry", async () => {
    const att = await createAttachmentForUser(userId, {
      entryId,
      fileName: "a.pdf",
      fileSize: 123,
      mimeType: "application/pdf",
      storageKey: "mock/" + userId + "/" + entryId + "/a.pdf",
    });
  ...
  it("rejects for non-owned entry", async () => {
    ...
    await expect(
      createAttachmentForUser(other.id, {
        entryId,
        fileName: "b.pdf",
        fileSize: 1,
        mimeType: "application/pdf",
