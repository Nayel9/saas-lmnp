...
vi.mock("../prisma", () => {
  ...
  interface AssetWhereInput {
    user_id?: string;
    acquisition_date?: { lte?: Date };
    label?: { contains: string; mode?: string };
  }
  interface FindManyArgs {
    where?: AssetWhereInput;
    orderBy?: { acquisition_date?: "asc" | "desc" };
  }
  function matches(a: MockAsset, where?: AssetWhereInput): boolean {
    if (!where) return true;
    if (where.user_id && a.user_id !== where.user_id) return false;
    if (
      where.acquisition_date?.lte &&
      !(a.acquisition_date <= where.acquisition_date.lte)
    )
      return false;
    return !(
      where.label?.contains &&
      !a.label.toLowerCase().includes(where.label.contains.toLowerCase())
    );
  }
  function findMany({ where, orderBy }: FindManyArgs): Promise<MockAsset[]> {
    ...
    if (orderBy?.acquisition_date) {
      list = list
        .slice()
        .sort(
          (a, b) => a.acquisition_date.getTime() - b.acquisition_date.getTime(),
        );
      if (orderBy.acquisition_date === "desc") list.reverse();
    }
    return Promise.resolve(list);
  }
  function create({ data }: { data: MockAsset }): Promise<MockAsset> {
    assets.push(data);
  ...
  function deleteMany({
    ...
    for (let i = assets.length - 1; i >= 0; i--)
      if (matches(assets[i], where)) assets.splice(i, 1);
  ...
  return {
    prisma: {
      asset: { findMany, create, deleteMany },
...
beforeAll(async () => {
  ...
  await prisma.asset.create({
    data: {
      id: "a1",
      user_id: userId,
      label: "Machine Avril 2024",
      amount_ht: 20000,
      duration_years: 10,
      acquisition_date: new Date("2024-04-01"),
      account_code: "215",
    },
  });
  ...
  await prisma.asset.create({
    data: {
      id: "a2",
      user_id: userId,
      label: "Mobilier Jan 2025",
      amount_ht: 3000,
      duration_years: 3,
      acquisition_date: new Date("2025-01-15"),
      account_code: "218",
    },
  });
});
...
describe("compute2033A", () => {
  it("année 2024: nettes = 20000 - 1500 = 18500", async () => {
    ...
    expect(r2024.actif_total).toBeCloseTo(
      r2024.capitaux_propres_equilibrage + r2024.deposits_held,
      2,
    );
  });
  it("année 2025: brut=23000 cumul=4500 net=18500 (3500 + 1000)", async () => {
    ...
    expect(r2025.actif_total).toBeCloseTo(
      r2025.capitaux_propres_equilibrage + r2025.deposits_held,
      2,
    );
  });
  it("cohérence partielle avec compute2033E (dotations)", async () => {
    ...
    const totalDotation = e2025.rows.reduce(
      (a, r) => a + r.dotation_exercice,
      0,
    );
