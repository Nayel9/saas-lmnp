...
vi.mock("../prisma", () => {
  ...
  interface ORCond {
    designation?: FieldFilter;
    tier?: FieldFilter;
    account_code?: FieldFilter;
  }
  interface DateFilter {
    gte?: Date;
    lte?: Date;
  }
  interface JEWhereInput {
    user_id?: string;
    date?: DateFilter;
    account_code?: FieldFilter;
    OR?: ORCond[];
  ...
  function matchWhere(e: MockJournalEntry, where?: JEWhereInput): boolean {
    ...
    return !(
      where.account_code?.contains &&
      !e.account_code
        .toLowerCase()
        .includes(where.account_code.contains.toLowerCase())
    );
  }
  function passesOR(e: MockJournalEntry, ors?: ORCond[]): boolean {
    ...
    return ors.some((cond) => {
      if (
        cond.designation?.contains &&
        e.designation
          .toLowerCase()
          .includes(cond.designation.contains.toLowerCase())
      )
        return true;
      if (
        cond.tier?.contains &&
        (e.tier || "").toLowerCase().includes(cond.tier.contains.toLowerCase())
      )
        return true;
      return !!(
        cond.account_code?.contains &&
        e.account_code
          .toLowerCase()
          .includes(cond.account_code.contains.toLowerCase())
      );
    });
  }
  function findMany(
    args: FindManyArgs,
  ): Promise<MockJournalEntry[] | Record<string, unknown>[]> {
    const { where, orderBy, skip, take, select } = args;
    let list = journal.filter(
      (j) => matchWhere(j, where) && passesOR(j, where?.OR),
    );
    if (orderBy?.date) {
      ...
      return Promise.resolve(
        list.map((entry) => {
          ...
          for (const k of Object.keys(select))
            if (select[k])
              o[k] = (entry as unknown as Record<string, unknown>)[k];
          return o;
        }),
      );
    }
  ...
  function deleteMany({
    ...
    for (let i = journal.length - 1; i >= 0; i--)
      if (matchWhere(journal[i], where)) journal.splice(i, 1);
  ...
  function createMany({
    ...
    for (const d of data) journal.push(d);
