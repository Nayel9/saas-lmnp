...
const RULES: RubriqueRule[] = (mapping as RubriqueRule[])
  .slice()
  .sort((a, b) => b.account_prefix.length - a.account_prefix.length);
 ...
 */
export function mapToRubriques(
  entries: JournalLikeEntry[],
  period?: PeriodFilter,
): MappingResult {
  const map = new Map<
    string,
    { rule: RubriqueRule; debit: number; credit: number }
  >();
  const amortizations: {
    date: string;
    amount: number;
  ...
  const rubriques: RubriqueAggregate[] = Array.from(map.values())
    .map(({ rule, debit, credit }) => ({
...
export function computeResultatCourant(rubriques: RubriqueAggregate[]): number {
  ...
  for (const r of rubriques) {
    if (r.rubrique === "CA")
      produits += r.total_credit - r.total_debit; // ventes nettes
    else if (r.rubrique === "CA_Moins")
      caMoins += r.total_debit - r.total_credit; // rabais (débits)
    else {
