...
export function listFor(type: AccountContext): CatalogAccount[] {
  return loadCatalog().filter((a) => a.appliesTo.includes(type));
}

// isAllowed: code présent ET contient le type dans appliesTo
export function isAllowed(code: string, type: AccountContext): boolean {
...
export function findClosest(
  ...
  let best: CatalogAccount | undefined;
  let bestLen = -1;
  for (const acc of catalog) {
    const c = acc.code;
    let match = 0;
    if (c.startsWith(input) || input.startsWith(c))
      match = Math.min(c.length, input.length);
    else if (c.slice(0, input.length - 1) === input.slice(0, input.length - 1))
      match = input.length - 1; // tolérance mineure
    if (match > bestLen) {
      best = acc;
      bestLen = match;
    }
