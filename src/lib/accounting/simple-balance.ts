...
export interface BalanceActif {
  vnc: number;
  treso: number;
  total: number;
}
export interface BalancePassif {
  cautions: number;
  dettes: number;
  total: number;
}
export interface SimpleBalanceResult {
  year: number;
  actif: BalanceActif;
  passif: BalancePassif;
  ecart: number;
}
...
export function computeCashAndDeposits(
  entries: SimpleEntry[],
  year: number,
): { cash: number; deposits: number } {
...
export function computeSimpleBalance(params: {
  assets: SimpleAsset[];
  entries: SimpleEntry[];
  year: number;
}): SimpleBalanceResult {
