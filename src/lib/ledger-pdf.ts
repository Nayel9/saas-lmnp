...
interface LedgerLineWithTier extends LedgerLine {
  tier?: string;
}
...
export async function generateLedgerPdf(
  ...
  const draw = (t: string, x: number, yy: number, size = 9) =>
    page.drawText(asciiSafe(t), { x, y: yy - size, size, font });
  const header = () => {
    y = A4_HEIGHT - TOP;
    draw(`${title} – Compte ${account_code}`, LEFT, y, 14);
    y -= LINE_H * 1.4;
    draw(`Période: ${period.from || "—"} -> ${period.to || "—"}`, LEFT, y, 9);
    y -= LINE_H;
    const f: string[] = [];
    if (filters.q) f.push(`q=${filters.q}`);
    if (f.length) {
      draw("Filtres: " + f.join(", "), LEFT, y, 9);
      y -= LINE_H;
    }
    const hy = y;
    y -= LINE_H;
