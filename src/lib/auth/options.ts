...
if (process.env.NODE_ENV === "production" && !process.env.NEXTAUTH_URL) {
  console.warn(
    "[auth] NEXTAUTH_URL n'est pas défini en production. Définissez-le pour des redirections correctes.",
  );
}
  ...
  callbacks: {
    ...
    async session({ session, token }) {
      ...
      if (!session.user) {
        ...
        (session as unknown as { user: Partial<SessionUser> }).user = {
          email: "",
          id: "",
        };
      }
    ...
    async jwt({ token, user, account, trigger }) {
      ...
      const isEdge =
        ...
        "EdgeRuntime" in (globalThis as object);
      if (
        !isEdge &&
        (trigger === "update" || !t.firstName || !t.lastName || !t.phone)
      ) {
