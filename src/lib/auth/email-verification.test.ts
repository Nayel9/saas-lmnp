...
interface MockStores { __stores: { users: { id: string; email: string; password?: string; role?: string; emailVerified?: Date | null }[]; vTokens: { identifier: string; token: string; expires: Date }[] } }
...
vi.mock('@/lib/prisma', () => {
  ...
  function update({ where: { email }, data }: { where: { email: string }; data: Partial<MockUser> }): Promise<MockUser> {
  ...
  const verificationToken = {
    findFirst: ({ where: { identifier, token } }: { where: { identifier: string; token: string } }) => Promise.resolve(vTokens.find(v=>v.identifier===identifier && v.token===token) || null),
  ...
  return { prisma: { user: { findUnique, update }, verificationToken, __stores: { users, vTokens } } };
});
vi.mock('bcryptjs', () => ({
  default: { compare: (pw: string, hash: string) => Promise.resolve(pw === hash) },
...
function makePost(body: unknown) {
  return new NextRequest('http://localhost/api/auth/resend-verification', { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json', 'x-forwarded-for': '1.2.3.4' } });
}
...
describe('Email verification helpers', () => {
  ...
  it('requireVerifiedCredentials retourne EMAIL_NOT_VERIFIED si emailVerified null', async () => {
    const stores = (prisma as unknown as MockStores).__stores;
    stores.users.push({ id: 'u1', email, password: 'Passw0rd!', role: 'user', emailVerified: null });
  ...
  it('requireVerifiedCredentials retourne user si vérifié', async () => {
    const stores = (prisma as unknown as MockStores).__stores;
    stores.users.push({ id: 'u2', email: 'ok@test.local', password: 'Passw0rd!', role: 'user', emailVerified: new Date() });
  ...
  it('resend-verification crée un token puis rate-limit empêche second', async () => {
    ...
    const stores = (prisma as unknown as MockStores).__stores;
    stores.users.push({ id: 'u3', email: 'rl@test.local', password: 'Passw0rd!', emailVerified: null });
    const now = Date.now();
  ...
  it('verify route valide token et marque emailVerified', async () => {
    const { token, hash, expires } = generateVerificationToken();
    const stores = (prisma as unknown as MockStores).__stores;
    stores.users.push({ id: 'u4', email: 'verif@test.local', password: 'Passw0rd!', emailVerified: null });
    stores.vTokens.push({ identifier: 'verif@test.local', token: hash, expires });
    const res = await verifyGET(makeGet(`http://localhost/auth/verify?token=${token}&email=verif@test.local`));
