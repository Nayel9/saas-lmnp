...
export function getS3Config() {
  ...
  if (!S3_REGION && !S3_ENDPOINT) throw new Error('S3_REGION ou S3_ENDPOINT requis');
  return {
...
export function buildStorageKey(params: { userId: string; entryId: string; fileName: string }) {
...
export async function presignPost(params: {
  ...
  const storageKey = buildStorageKey({ userId: params.userId, entryId: params.entryId, fileName: params.fileName });
...
export async function presignGet(params: { storageKey: string; expiresSec?: number }): Promise<string> {
  const client = getS3Client();
  const cmd = new GetObjectCommand({ Bucket: getS3Config().bucket, Key: params.storageKey });
  return await getSignedUrl(client, cmd, { expiresIn: params.expiresSec ?? 120 });
}

export async function presignGetWithDisposition(params: { storageKey: string; expiresSec?: number; disposition: string }): Promise<string> {
  const client = getS3Client();
  const cmd = new GetObjectCommand({ Bucket: getS3Config().bucket, Key: params.storageKey, ResponseContentDisposition: params.disposition });
  return await getSignedUrl(client, cmd, { expiresIn: params.expiresSec ?? 120 });
}
