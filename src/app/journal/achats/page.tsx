...
import JournalAchatsClient from './ui-client';
...
export default async function JournalAchatsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const where: Prisma.JournalEntryWhereInput = { user_id: user.id, type: 'achat' };
  ...
  const [total, entries, tierSuggestions, accountSuggestions] = await Promise.all([
    ...
    prisma.journalEntry.findMany({ where: { user_id: user.id, type: 'achat', tier: { not: null } }, distinct: ['tier'], select: { tier: true }, take: 20 }),
    prisma.journalEntry.findMany({ where: { user_id: user.id, type: 'achat' }, distinct: ['account_code'], select: { account_code: true }, take: 20 })
  ]);
  ...
  return (
    <main className="p-6 max-w-6xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Journal Achats</h1>
      ...
      <section className="card p-4 space-y-4">
        <form className="grid md:grid-cols-6 gap-3 text-sm" method="get">
          ...
          <input name="tier" list="tiers-achats" aria-label="Tier" defaultValue={(sp.tier as string)||''} className="input" placeholder="Tier" />
          <datalist id="tiers-achats">{tierSuggestions.filter(t=>t.tier).map(t=> <option key={t.tier}>{t.tier}</option>)}</datalist>
          <input name="account_code" list="accounts-achats" aria-label="Compte comptable" defaultValue={(sp.account_code as string)||''} className="input" placeholder="Compte" />
          <datalist id="accounts-achats">{accountSuggestions.map(a=> <option key={a.account_code}>{a.account_code}</option>)}</datalist>
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            ...
            {hasActiveFilter && <a href="/journal/achats" className="text-xs underline">Réinitialiser</a>}
          </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          ...
          <a className="btn text-xs" href={`/api/journal/achats/export?format=pdf${sp.from?`&from=${sp.from}`:''}${sp.to?`&to=${sp.to}`:''}${sp.tier?`&tier=${sp.tier}`:''}${sp.account_code?`&account_code=${sp.account_code}`:''}${sp.q?`&q=${sp.q}`:''}`}>Export PDF</a>
        </div>
      </section>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          <Link className="btn" href="/api/journal/achats/export?format=csv">Export CSV</Link>
          <Link className="btn" href="/api/journal/achats/export?format=xlsx">Export XLSX</Link>
        </div>
      </div>
    </main>
  );
