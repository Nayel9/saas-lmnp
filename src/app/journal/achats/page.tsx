...
export default async function JournalAchatsPage({
  ...
  const [total, entries, tierSuggestions, accountSuggestions, properties] =
    await Promise.all([
      ...
      prisma.journalEntry.findMany({
        where: { user_id: user.id, type: "achat", tier: { not: null } },
        distinct: ["tier"],
        select: { tier: true },
        take: 20,
      }),
      prisma.journalEntry.findMany({
        where: { user_id: user.id, type: "achat" },
  ...
  const totals = computeVisibleTotals(
    entries.map((e) => ({ amount: Number(e.amount) })),
  );
  ...
  return (
    <main className="p-6 max-w-6xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        ...
        <JournalAchatsClient properties={properties} />
      </header>
      ...
      <section className="card p-4 space-y-4">
        <form className="grid md:grid-cols-6 gap-3 text-sm" method="get">
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            ...
            {hasActiveFilter && (
              <a href="/journal/achats" className="text-xs underline">
                Réinitialiser
              </a>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          ...
          <span>
            Total: <strong>{formatAmount(totals.sum)}</strong>
          </span>
          <a
            className="btn text-xs"
            href={`/api/journal/achats/export?format=pdf${sp.from ? `&from=${sp.from}` : ""}${sp.to ? `&to=${sp.to}` : ""}${sp.tier ? `&tier=${sp.tier}` : ""}${sp.account_code ? `&account_code=${sp.account_code}` : ""}${sp.q ? `&q=${sp.q}` : ""}`}
          >
            Export PDF
          </a>
        </div>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          <Link className="btn" href="/api/journal/achats/export?format=csv">
            Export CSV
          </Link>
          <Link className="btn" href="/api/journal/achats/export?format=xlsx">
            Export XLSX
          </Link>
        </div>
      </div>
