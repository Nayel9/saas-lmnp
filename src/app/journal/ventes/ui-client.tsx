...
async function presign(entryId: string, file: File) {
  const res = await fetch('/api/uploads/presign', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ fileName: file.name, fileSize: file.size, mimeType: file.type || 'application/octet-stream', entryId }) });
  if (!res.ok) throw new Error('Pré-signature échouée');
  return await res.json() as Promise<{ provider: 's3'|'mock'; url: string; fields?: Record<string,string>; headers?: Record<string,string>; storageKey: string }>;
}
...
async function createAttachment(payload: { entryId: string; fileName: string; fileSize: number; mimeType: string; storageKey: string }) {
  ...
  async function uploadAll(entryId: string) {
    for (const f of files) {
      try {
        ...
        await createAttachment({ entryId, fileName: f.name, fileSize: f.size, mimeType: f.type || 'application/octet-stream', storageKey: pre.storageKey });
      } catch {
            ...
            <form onSubmit={onSubmit} className="space-y-3">
              ...
              <div className="mt-2 border rounded-md p-3">
                ...
                <div onDragOver={(e)=>e.preventDefault()} onDrop={onDrop} className="text-xs text-muted-foreground border border-dashed rounded p-3">
                  ...
                  <div className="mt-2">
                    <button type="button" className="btn inline-flex items-center gap-2" onClick={()=>fileInputRef.current?.click()} disabled={isPending}>
                      {isPending && <Spinner />}
                      Choisir un fichier
                    </button>
                    <input ref={fileInputRef} type="file" accept="application/pdf,image/jpeg,image/png" hidden onChange={(e)=>{ if(e.target.files) onFiles(e.target.files); }} />
                  </div>
                </div>
                {!!files.length && (
                  <ul className="mt-2 text-xs list-disc pl-4">
                    {files.map((f,i)=>(<li key={i}>{f.name} ({(f.size/1024).toFixed(0)} Ko)</li>))}
                  </ul>
                )}
              </div>
