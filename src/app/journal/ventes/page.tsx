...
export default async function JournalVentesPage({
  ...
  if (sp.from)
    (where.date as Prisma.DateTimeFilter).gte = new Date(sp.from as string);
  if (sp.to)
    (where.date as Prisma.DateTimeFilter).lte = new Date(sp.to as string);
  if (sp.tier)
    where.tier = { contains: sp.tier as string, mode: "insensitive" };
  if (sp.q) {
    where.OR = [
  ...
  if (sp.account_code)
    where.account_code = {
      contains: sp.account_code as string,
      mode: "insensitive",
    };
  ...
  const [total, entries, tierSuggestions, accountSuggestions, properties] =
    await Promise.all([
      ...
      prisma.journalEntry.findMany({
        where: { user_id: user.id, type: "vente", tier: { not: null } },
        distinct: ["tier"],
        select: { tier: true },
        take: 20,
      }),
      prisma.journalEntry.findMany({
        where: { user_id: user.id, type: "vente" },
  ...
  const totals = computeVisibleTotals(
    visibleForTotals.map((e) => ({ amount: Number(e.amount) })),
  );
  ...
  return (
    <main className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <section className="card p-4 space-y-4">
        <form className="grid md:grid-cols-7 gap-3 text-sm" method="get">
          <input
            ...
            defaultValue={(sp.from as string) || ""}
            className="input"
            placeholder="De"
          />
          <input
          ...
          <input
            ...
            list="tiers-ventes"
            aria-label="Client"
            defaultValue={(sp.tier as string) || ""}
            className="input"
            placeholder="Client"
          />
          <datalist id="tiers-ventes">
            {tierSuggestions
              .filter((t) => t.tier)
              .map((t) => (
          ...
          <input
            name="account_code"
            list="accounts-ventes"
            aria-label="Compte comptable"
            defaultValue={(sp.account_code as string) || ""}
            className="input"
            placeholder="Compte"
          />
          <datalist id="accounts-ventes">
            {accountSuggestions.map((a) => (
              <option key={a.account_code}>{a.account_code}</option>
            ))}
          </datalist>
          <select
            name="deposit"
            aria-label="Caution"
            defaultValue={depositFilter}
            className="input"
          >
          ...
          <div className="flex items-center gap-2 md:col-span-2">
            ...
            {hasActiveFilter && (
              <a href="/journal/ventes" className="text-xs underline">
                Réinitialiser
              </a>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-4 text-sm">
          <span>
            <strong>{totals.count}</strong> lignes visibles
          </span>
          <span>
            Total (hors cautions): <strong>{formatAmount(totals.sum)}</strong>
          </span>
          <a
            className="btn text-xs"
            href={`/api/journal/ventes/export?format=pdf${sp.from ? `&from=${sp.from}` : ""}${sp.to ? `&to=${sp.to}` : ""}${sp.tier ? `&tier=${sp.tier}` : ""}${sp.account_code ? `&account_code=${sp.account_code}` : ""}${sp.q ? `&q=${sp.q}` : ""}`}
          >
            Export PDF
          </a>
        </div>
            ...
            {entries.map((e) => (
              ...
              >
                ...
                <td className="py-1 pr-4">
                  ...
                  {e.isDeposit && (
                    <span className="badge ml-2" title="Exclue du revenu">
                      Caution
                    </span>
                  )}
                </td>
      ...
      <div className="flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          <Link className="btn" href="/api/journal/ventes/export?format=csv">
            Export CSV
          </Link>
          <Link className="btn" href="/api/journal/ventes/export?format=xlsx">
            Export XLSX
          </Link>
        </div>
      </div>
