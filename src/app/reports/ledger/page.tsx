...
async function fetchLedger(
  userId: string,
  account_code: string,
  params: { from?: string | null; to?: string | null; q?: string | null },
) {
  ...
  if (q) {
    whereParts.push(`(designation ILIKE $${idx} OR tier ILIKE $${idx})`);
    values.push("%" + q + "%");
    idx++;
  }
    ...
    return (
      <div className="p-8 space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Paramètre account_code requis. Retour à la{" "}
          <Link href="/reports/balance" className="underline">
            Balance
          </Link>
          .
        </p>
    ...
    <main className="p-6 max-w-6xl mx-auto space-y-6">
      <header className="flex flex-col gap-1">
        <h1 className="text-2xl font-semibold tracking-tight">
          Grand livre – {account_code}
        </h1>
        <p className="text-sm text-muted-foreground">
          {enriched.length} écritures • Débit {totalDebit.toFixed(2)} • Crédit{" "}
          {totalCredit.toFixed(2)} • Solde{" "}
      ...
      <section className="card p-4 space-y-4 text-sm">
        <form className="grid md:grid-cols-6 gap-3" method="get">
          ...
          <div className="flex items-center gap-2 md:col-span-1">
            ...
            {hasFilter && (
              <a
                href={`/reports/ledger?account_code=${encodeURIComponent(account_code)}`}
        ...
        <div className="flex gap-2">
          <a
            ...
            href={`/api/reports/ledger/export?format=csv&account_code=${encodeURIComponent(account_code)}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}${q ? `&q=${q}` : ""}`}
          >
            Export CSV
          </a>
          <a
            className="btn text-xs"
            href={`/api/reports/ledger/export?format=pdf&account_code=${encodeURIComponent(account_code)}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}${q ? `&q=${q}` : ""}`}
          >
            Export PDF
          </a>
        </div>
