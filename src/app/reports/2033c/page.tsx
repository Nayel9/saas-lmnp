...
export default async function C2033CPage({
  searchParams,
}: {
  ...
  if (getUserRole(user) !== "admin")
    return <div className="p-8">Accès administrateur requis</div>;
  const data = await compute2033C({
    userId: user.id,
    from,
    to,
    q,
  ...
  const rubriqueMap = Object.fromEntries(
    data.rubriques.map((r) => [r.rubrique, r]),
  );
  const get = (code: string) => rubriqueMap[code];

  const produitsRows = [get("CA"), get("CA_Moins")].filter(Boolean);
  const chargesRows = data.rubriques.filter(
    (r) => !["CA", "CA_Moins", "DotationsAmortissements"].includes(r.rubrique),
  );
  ...
  type RRow =
    ...
    | undefined;
  function montantProduit(r: RRow) {
    if (!r) return 0;
    if (r.rubrique === "CA") return r.total_credit - r.total_debit;
    if (r.rubrique === "CA_Moins") return -(r.total_debit - r.total_credit);
    return 0;
  }
  function montantCharge(r: RRow) {
    if (!r) return 0;
    return r.total_debit - r.total_credit;
  }
    ...
    <main className="p-6 max-w-5xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Agrégation simplifiée (LMNP)
          </p>
        </div>
        <div className="flex gap-2">
          <a
            className="btn text-xs"
            href={`/api/reports/2033c/export?format=xlsx${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}${q ? `&q=${q}` : ""}${account_code ? `&account_code=${account_code}` : ""}`}
          >
            Export XLSX
          </a>
        </div>
      ...
      <section className="card p-4 space-y-4 text-sm">
        <form className="grid md:grid-cols-6 gap-3" method="get">
          ...
          <input
            name="q"
            placeholder="Recherche (designation / tier / compte)"
            defaultValue={q || ""}
          ...
          <div className="flex items-center gap-2 md:col-span-1">
            ...
            {hasFilters && (
              <a href="/reports/2033c" className="text-xs underline">
                Reset
              </a>
            )}
          </div>
      ...
      <section className="card p-4 overflow-x-auto text-sm space-y-6">
        ...
        <div>
          ...
          <table className="w-full mb-2">
            <tbody>
              {produitsRows.map((r) => (
                <tr key={r.rubrique} className="border-b last:border-none">
                  <td className="py-1 pr-4">{r.label}</td>
                  <td className="py-1 text-right tabular-nums">
                    {montantProduit(r).toFixed(2)}
                  </td>
                </tr>
              ))}
              {!produitsRows.length && (
              ...
              <tr className="font-medium">
                ...
                <td className="py-1 text-right tabular-nums">
                  {data.totals.produits.toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table className="w-full mb-2">
            <tbody>
              ...
              <tr className="font-medium">
                ...
                <td className="py-1 text-right tabular-nums">
                  {data.totals.amortissements.toFixed(2)}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <div className="text-sm font-semibold flex justify-between border-t pt-3">
          ...
          <span className="tabular-nums">
            {data.totals.resultat.toFixed(2)}
          </span>
        </div>
        {data.totals.resultat < 0 && (
          <div className="text-xs text-[--color-danger]">Résultat négatif</div>
        )}
        {data.truncated && (
          <div className="text-xs text-[--color-warning]">
            Données tronquées (volume &gt; limite interne)
          </div>
        )}
        ...
        <div className="text-xs text-muted-foreground flex flex-wrap gap-4">
          ...
          <Link href="/reports/2033e" className="underline">
            Aller 2033-E (amortissements)
          </Link>
        </div>
      </section>
    </main>
