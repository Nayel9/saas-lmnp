...
export default async function C2033APage({
  searchParams,
}: {
    ...
    <main className="p-6 max-w-4xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <a
            ...
            href={`/api/reports/2033a/export?year=${year}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
          >
            Export XLSX
          </a>
        </div>
      ...
      <section className="card p-4 space-y-4 text-sm">
        <form className="flex flex-wrap gap-3 items-end" method="get">
          ...
          {(yearStr || q) && (
            <a href="/reports/2033a" className="underline text-xs">
              Reset
            </a>
          )}
          <div className="text-xs text-muted-foreground ml-auto">
            {d.count_assets} actifs (brut {d.immobilisations_brutes.toFixed(2)}){" "}
            {d.truncated && "(tronqué)"}{" "}
          </div>
        </form>
      </section>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="card p-4 text-sm">
          ...
          <table className="w-full">
            <tbody>
              <tr>
                <td className="py-1 pr-4">Immobilisations nettes</td>
                <td className="py-1 text-right tabular-nums">
                  {d.immobilisations_nettes.toFixed(2)}
                </td>
              </tr>
              <tr>
                <td className="py-1 pr-4">Trésorerie (v1)</td>
                <td className="py-1 text-right tabular-nums">
                  {d.tresorerie.toFixed(2)}
                </td>
              </tr>
            </tbody>
            <tfoot>
          ...
          <table className="w-full">
            <tbody>
              <tr>
                ...
                <td className="py-1 text-right tabular-nums">
                  {d.deposits_held.toFixed(2)}
                </td>
              </tr>
              <tr>
                <td className="py-1 pr-4">Capitaux propres (équilibrage v1)</td>
                <td className="py-1 text-right tabular-nums">
                  {d.capitaux_propres_equilibrage.toFixed(2)}
                </td>
              </tr>
            </tbody>
            <tfoot>
      ...
      <div className="text-xs flex items-center gap-3">
        ...
        <span className="text-muted-foreground">
          (Nettes = Brut - Amort. cumulés au 31/12)
        </span>
      </div>
    </main>
  );
