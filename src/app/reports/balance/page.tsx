      ...
      <header className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {rows.length} comptes agrégés
          </p>
        </div>
        <div className="flex gap-2">
          <a
            className="btn text-xs"
            href={`/api/reports/balance/export?format=csv${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}${account_code ? `&account_code=${account_code}` : ""}${q ? `&q=${q}` : ""}`}
          >
            Export CSV
          </a>
          <a
            className="btn text-xs"
            href={`/api/reports/balance/export?format=pdf${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}${account_code ? `&account_code=${account_code}` : ""}${q ? `&q=${q}` : ""}`}
          >
            Export PDF
          </a>
        </div>
      ...
      <section className="card p-4 space-y-4 text-sm">
        <form className="grid md:grid-cols-6 gap-3" method="get">
          ...
          <div className="flex items-center gap-2 md:col-span-1">
            ...
            {hasActiveFilter && (
              <a href="/reports/balance" className="text-xs underline">
                Reset
              </a>
            )}
          </div>
          <div className="md:col-span-6 flex flex-wrap gap-4 mt-2 text-xs text-muted-foreground">
            ...
            <span>
              Totaux: Débit {total_debit.toFixed(2)} · Crédit{" "}
              {total_credit.toFixed(2)} · Solde {total_balance.toFixed(2)}
            </span>
          </div>
        </form>
      </section>
            ...
            {rows.map((r) => (
              ...
              >
                ...
                <td className="py-1 pr-2 text-right text-xs">
                  <Link
                    href={`/reports/ledger?account_code=${encodeURIComponent(r.account_code)}${from ? `&from=${from}` : ""}${to ? `&to=${to}` : ""}`}
                    className="underline"
                  >
                    Grand livre
                  </Link>
