...
export default async function C2033EPage({
  searchParams,
}: {
    ...
    <main className="p-6 max-w-6xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold">
            État des amortissements 2033-E
          </h1>
        ...
        <div className="flex gap-2">
          <a
            ...
            href={`/api/reports/2033e/export?year=${year}${q ? `&q=${encodeURIComponent(q)}` : ""}`}
          >
            Export XLSX
          </a>
        </div>
      ...
      <section className="card p-4 space-y-4 text-sm">
        <form className="flex flex-wrap gap-3 items-end" method="get">
          ...
          {(yearStr || q) && (
            <a href="/reports/2033e" className="underline text-xs">
              Reset
            </a>
          )}
          <div className="text-xs text-muted-foreground ml-auto">
            {data.rows.length} actifs affichés{" "}
            ...
            {data.rows.map((r) => (
              ...
              >
                ...
                <td className="py-1 pr-4 text-right tabular-nums">
                  {r.valeur_origine.toFixed(2)}
                </td>
                <td className="py-1 pr-4 text-right tabular-nums">
                  {r.amortissements_anterieurs.toFixed(2)}
                </td>
                <td className="py-1 pr-4 text-right tabular-nums">
                  {r.dotation_exercice.toFixed(2)}
                </td>
                <td className="py-1 pr-4 text-right tabular-nums">
                  {r.amortissements_cumules.toFixed(2)}
                </td>
                <td className="py-1 pr-2 text-right tabular-nums">
                  {r.valeur_nette.toFixed(2)}
                </td>
              </tr>
            ))}
            {!data.rows.length && (
          ...
          <tfoot>
            <tr className="font-semibold">
              ...
              <td className="py-2 pr-4 text-right">
                {data.totals.valeur_origine.toFixed(2)}
              </td>
              <td className="py-2 pr-4 text-right">
                {data.totals.amortissements_anterieurs.toFixed(2)}
              </td>
              <td className="py-2 pr-4 text-right">
                {data.totals.dotation_exercice.toFixed(2)}
              </td>
              <td className="py-2 pr-4 text-right">
                {data.totals.amortissements_cumules.toFixed(2)}
              </td>
              <td className="py-2 pr-2 text-right">
                {data.totals.valeur_nette.toFixed(2)}
              </td>
            </tr>
          </tfoot>
        </table>
