    ...
    <main className="p-6 max-w-3xl mx-auto space-y-6">
      <header className="space-y-2">
        ...
        <nav className="flex gap-2" aria-label="Onglets synthèse">
          <a
            href={makeHref("result")}
            className={`px-3 py-1.5 rounded-md text-sm ${tab === "result" ? "bg-bg-muted text-brand" : "hover:bg-bg-muted"}`}
          >
            Résultat (simple)
          </a>
          <a
            href={makeHref("balance")}
            className={`px-3 py-1.5 rounded-md text-sm ${tab === "balance" ? "bg-bg-muted text-brand" : "hover:bg-bg-muted"}`}
          >
            Bilan (simple)
          </a>
        </nav>
