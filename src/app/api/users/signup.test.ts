...
vi.mock("@/lib/prisma", () => {
  ...
  function create({
    ...
    data: Omit<MockUser, "id"> & { id?: string };
  }): Promise<MockUser> {
    const user: MockUser = {
      id: data.id || `u_${users.length}`,
      email: data.email,
  ...
  return {
    prisma: { user: { findUnique, create, deleteMany }, verificationToken },
  };
});
...
describe("POST /api/users (signup)", () => {
  ...
  it("retourne 409 si email déjà existant", async () => {
    const res = await POST(
      makeReq(
        { email, password, firstName, lastName, phone, acceptTerms: true },
        "127.0.0.2",
      ),
    );
  ...
  it("retourne 400 si champs profil requis manquants", async () => {
    const res = await POST(
      makeReq(
        { email: `x-${email}`, password, acceptTerms: true },
        "127.0.0.4",
      ),
    );
    expect(res.status).toBe(400);
