...
describe('POST /api/users (signup)', () => {
  ...
  it('retourne 409 si email déjà existant', async () => {
    const res = await POST(makeReq({ email, password, firstName, lastName, phone, acceptTerms: true }, '127.0.0.2'));
    expect(res.status).toBe(409);
  });
  it('retourne 400 si payload invalide', async () => {
    const res = await POST(makeReq({ email: 'not-an-email', password: 'short', firstName: '', lastName: '', phone: 'abc', acceptTerms: true }, '127.0.0.3'));
    expect(res.status).toBe(400);
  });
  it('retourne 400 si champs profil requis manquants', async () => {
    const res = await POST(makeReq({ email: `x-${email}`, password, acceptTerms: true }, '127.0.0.4'));
    expect(res.status).toBe(400);
  });
  it('normalise un téléphone FR vers E.164', async () => {
    const emailNorm = `norm-${Date.now()}@local.test`;
    const res = await POST(makeReq({ email: emailNorm, password, firstName, lastName, phone: '06 12 34 56 78', acceptTerms: true }, '10.0.0.1'));
    expect(res.status).toBe(201);
