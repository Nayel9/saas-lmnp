...
async function fetchAggregated(userId: string, params: { from?: string|null; to?: string|null; account_code?: string|null; q?: string|null }) {
...
export async function GET(req: NextRequest) {
  ...
  if (format === 'csv') {
    ...
    const body = rows.map(r => [r.account_code, r.total_debit.toFixed(2), r.total_credit.toFixed(2), r.balance.toFixed(2)].join(';')).join('\n');
    return new Response(header + body + (body?'\n':''), { headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename="balance.csv"' }});
  }
  // PDF (troncation si >5000)
  const MAX_PDF_ROWS = 5000;
  const truncated = rows.length > MAX_PDF_ROWS;
  const pdfBuf = await generateBalancePdf({ rows: truncated ? rows.slice(0, MAX_PDF_ROWS) : rows, period: { from, to }, filters: { account_code, q } });
  return new Response(new Uint8Array(pdfBuf), { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="balance.pdf"', ...(truncated? { 'X-Truncated': 'true' }: {}) }});
}
