...
export async function GET(req: NextRequest) {
  ...
  const actifRows = [
    {
      Poste: "Immobilisations nettes",
      Montant: r.immobilisations_nettes.toFixed(2),
    },
    { Poste: "Trésorerie (v1)", Montant: r.tresorerie.toFixed(2) },
    { Poste: "TOTAL ACTIF", Montant: r.actif_total.toFixed(2) },
  ];
  const totalPassif = (
    r.deposits_held + r.capitaux_propres_equilibrage
  ).toFixed(2);
  const passifRows = [
    { Poste: "Cautions détenues", Montant: r.deposits_held.toFixed(2) },
    {
      Poste: "Capitaux propres (équilibrage v1)",
      Montant: r.capitaux_propres_equilibrage.toFixed(2),
    },
