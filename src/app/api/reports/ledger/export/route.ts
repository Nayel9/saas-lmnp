...
export async function GET(req: NextRequest) {
  ...
  if (!account_code)
    return new Response("account_code requis", { status: 400 });
  ...
  const ledgerLines = computeLedger(
    baseRows.map((r) => ({
      date: r.date,
      account_code,
      designation: r.designation,
  ...
  const ledgerWithTier = ledgerLines.map((l, i) => ({
    ...l,
    tier: baseRows[i]?.tier || "",
  }));

  if (format === "csv") {
    const header = "date;designation;tier;debit;credit;balance\n";
    const body = ledgerWithTier
      .map((l) =>
        [
          l.date.slice(0, 10),
    ...
    return new Response(header + body + (body ? "\n" : ""), {
      headers: {
        ...
        "Content-Disposition": `attachment; filename=ledger_${account_code}.csv`,
      },
    });
  }
  const MAX = 5000;
  const truncated = ledgerLines.length > MAX;
  const pdfBuf = await generateLedgerPdf({
    account_code,
    rows: truncated ? ledgerLines.slice(0, MAX) : ledgerLines,
    period: { from, to },
    filters: { q },
  });
  return new Response(new Uint8Array(pdfBuf), {
    headers: {
      ...
      "Content-Disposition": `attachment; filename=ledger_${account_code}.pdf`,
