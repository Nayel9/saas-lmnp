...
function buildWhere(
  ...
  if (account_code)
    where.account_code = { contains: account_code, mode: "insensitive" };
  return where;
}
...
export async function GET(req: NextRequest) {
  ...
  if (format === "csv") {
    const header =
      "date;designation;client;account_code;amount;currency;isDeposit\n";
    const body = rows
      .map((r) =>
        [
    ...
    return new Response(header + body + (body ? "\n" : ""), {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="journal-ventes.csv"',
      },
    });
  }
  if (format === "pdf") {
    ...
    return new Response(new Uint8Array(pdfBuf), {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="journal-ventes.pdf"',
      },
    });
  }
  // XLSX
  ...
  return new Response(new Uint8Array(buf), {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="journal-ventes.xlsx"',
    },
  });
}
