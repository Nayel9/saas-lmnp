...
function buildWhere(userId: string, params: { from?: string|null; to?: string|null; tier?: string|null; q?: string|null; account_code?: string|null; type: 'vente' }) : Prisma.JournalEntryWhereInput {
...
export async function GET(req: NextRequest) {
  ...
  const where = buildWhere(user.id, { from, to, tier, q, account_code, type: 'vente' });
  ...
  if (format === 'pdf') {
    ...
    return new Response(new Uint8Array(pdfBuf), { headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="journal-ventes.pdf"' }});
  }
  // XLSX
  ...
  return new Response(new Uint8Array(buf), { headers: { 'Content-Type': 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'Content-Disposition': 'attachment; filename="journal-ventes.xlsx"' }});
}
