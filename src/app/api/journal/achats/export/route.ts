...
export async function GET(req: NextRequest) {
  ...
  if (format === "csv") {
    ...
    return new Response(header + body + (body ? "\n" : ""), {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="journal-achats.csv"',
      },
    });
  }
  if (format === "pdf") {
    ...
    return new Response(new Uint8Array(pdfBuf), {
      headers: {
        ...
        "Content-Disposition": 'attachment; filename="journal-achats.pdf"',
      },
    });
  }
  // XLSX
  ...
  return new Response(new Uint8Array(buf), {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="journal-achats.xlsx"',
    },
  });
}
