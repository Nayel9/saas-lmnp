...
export async function GET(req: NextRequest) {
  ...
  const idParse = UUID.safeParse(p); if (!idParse.success) return new Response('BAD_REQUEST: property invalide', { status: 400 });
  const yearParse = YEAR.safeParse(y ?? String(new Date().getFullYear())); if (!yearParse.success) return new Response('BAD_REQUEST: year invalide', { status: 400 });
  const propertyId = idParse.data; const year = yearParse.data;

  // Propriété et contrôle multi-tenant
  const property = await prisma.property.findUnique({ where: { id: propertyId } });
  if (!property || property.user_id !== user.id) return new Response('FORBIDDEN: propriété inconnue', { status: 403 });

  const start = new Date(Date.UTC(year,0,1));
  const end = new Date(Date.UTC(year,11,31,23,59,59,999));

  const entries = await prisma.journalEntry.findMany({ where: { user_id: user.id, date: { gte: start, lte: end } }, orderBy: { date: 'asc' } });
  const assets = await prisma.asset.findMany({ where: { user_id: user.id, acquisition_date: { lte: end } }, orderBy: { acquisition_date: 'asc' } });
  ...
  const assetsSimple = assets.map(a => ({ amount_ht: Number(a.amount_ht), duration_years: a.duration_years, acquisition_date: new Date(a.acquisition_date) }));
  const entriesSimple = entries.map(e => ({ type: e.type, amount: Number(e.amount), date: new Date(e.date), isDeposit: e.isDeposit })) as { type: 'achat'|'vente'; amount: number; date: Date; isDeposit?: boolean }[];
  const balance = computeSimpleBalance({ assets: assetsSimple, entries: entriesSimple, year });
  ...
  const header = buildPdfTextModel({
    ...
    year,
    income,
    balance: { actif: { vnc: balance.actif.vnc, treso: balance.actif.treso }, passif: { cautions: balance.passif.cautions, dettes: balance.passif.dettes }, totals: { actif: balance.actif.total, passif: balance.passif.total } },
  });
  ...
  drawText(`Immobilisations (valeur restante): ${formatEUR(balance.actif.vnc)}`);
