...
vi.mock('@/lib/prisma', () => {
  ...
  const journalEntry = { findMany: vi.fn(async () => {
    return [
      { type: 'vente', amount: 1000, isDeposit: false, date: new Date('2025-03-10') },
      { type: 'vente', amount: 200, isDeposit: true, date: new Date('2025-04-01') },
      { type: 'achat', amount: 300, isDeposit: false, date: new Date('2025-02-11') },
      { type: 'achat', amount: 50, isDeposit: false, date: new Date('2025-05-20') },
      { type: 'achat', amount: 999, isDeposit: false, date: new Date('2026-01-01') },
    ];
  }) };
...
describe('GET /api/synthesis/balance', () => {
  it('retourne un bilan simple cohérent (VNC, trésorerie, cautions)', async () => {
    const res = await GET(makeReq(`http://test/api/synthesis/balance?property=${P1}&year=2025`));
  ...
  it('contrôle multi-tenant: forbidden si propriété autre user', async () => {
    const res = await GET(makeReq(`http://test/api/synthesis/balance?property=${P2}&year=2025`));
    expect(res.status).toBe(403);
  });
  it('valide year / property requis', async () => {
    const r1 = await GET(makeReq('http://test/api/synthesis/balance?year=2025'));
    expect(r1.status).toBe(400);
    const r2 = await GET(makeReq(`http://test/api/synthesis/balance?property=${P1}&year=1999`));
