...
import { buildIndexCsv, buildZipPath, type IndexRow } from '@/lib/exports/attachmentsIndex';
...
export async function GET(req: NextRequest) {
  ...
  const attachments = ids.length ? await prisma.attachment.findMany({ where: { entryId: { in: ids } } }) : [];
  ...
  archive.on('warning', (err: unknown) => { console.warn('ZIP warn', err); });
  archive.on('error', (err: unknown) => { pass.destroy(err as Error); });
  archive.pipe(pass);
  ...
  for (const att of attachments) {
    ...
    if (!e) continue;
    const dateISO = e.date.toISOString().slice(0, 10);
    const zipPath = buildZipPath({ type: e.type, dateISO, entryId: e.id, fileName: att.fileName });
    try {
  ...
  const toWeb = (Readable as unknown as { toWeb?: (s: NodeJS.ReadableStream) => ReadableStream }).toWeb;
  const body = toWeb ? toWeb(pass) : (pass as unknown as ReadableStream);
  console.log('Export pièces ZIP', { userId: user.id, propertyId, from, to, count: attachments.length });
