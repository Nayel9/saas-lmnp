...
import { presignGetWithDisposition } from '@/lib/storage/s3';
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const url = await presignGetWithDisposition({ storageKey: att.storageKey, expiresSec: 120, disposition: `inline; filename="${att.fileName}"` });
