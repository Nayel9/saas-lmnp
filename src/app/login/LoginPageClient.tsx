...
const signupSchema = loginSchema.extend({
    ...
    phone: z.string().trim().min(5, 'Requis').max(30).regex(/^[+0-9 ()-]*$/, 'Format invalide')
...
export default function LoginPageClient() {
    ...
    if (status === 'loading') {
        return <main className="min-h-screen flex items-center justify-center"><p
            className="text-sm text-muted-foreground">Chargement…</p></main>;
    }
    ...
    return (
        <main id="main" className="min-h-screen flex flex-col items-center px-4 py-10">
            <div className="w-full max-w-md card" role="region" aria-labelledby="auth-title">
                ...
                {verifiedParam === '1' && <div
                    className="mb-4 rounded-md bg-green-50 border border-green-200 p-3 text-xs text-green-700">Email
                    vérifié, vous pouvez maintenant vous connecter.</div>}
                {verifiedParam === '0' &&
                    <div className="mb-4 rounded-md bg-red-50 border border-red-200 p-3 text-xs text-red-600">Lien
                        invalide ou expiré, renvoyez un email de vérification.</div>}
                {errors.global &&
                    <div className="mb-4 rounded-md bg-red-50 border border-red-200 p-3 text-xs text-red-600"
                         role="alert">{errors.global}</div>}
                {successMessage &&
                    <div className="mb-4 rounded-md bg-green-50 border border-green-200 p-3 text-xs text-green-700"
                         role="status">{successMessage}</div>}
                <form onSubmit={submit} className="flex flex-col gap-4" noValidate>
                    <div aria-hidden="true" style={{
                        ...
                        <input id="website" name="website" type="text" tabIndex={-1} autoComplete="off"
                               value={honeyValue} onChange={onChange}/>
                    </div>
                    <InputField id="email" label="Email" type="email" value={formData.email} onChange={onChange}
                                disabled={loading} error={errors.email} placeholder="vous@exemple.fr"
                                autoComplete="email"/>
                    <PasswordField id="password" label="Mot de passe" value={formData.password} onChange={onChange}
                                   disabled={loading} error={errors.password} placeholder="••••••••"
                                   autoComplete={mode === 'login' ? 'current-password' : 'new-password'}/>
                    {showStrength && <PasswordStrengthMeter password={formData.password} className="bg-white"/>}
                    {mode === 'signup' && <PasswordField id="confirmPassword" label="Confirmer le mot de passe"
                    ...
                    {mode === 'signup' && (
                        <>
                            <InputField id="firstName" label="Prénom" value={formData.firstName} onChange={onChange}
                                        disabled={loading} error={errors.firstName} placeholder="Votre prénom"
                                        autoComplete="given-name"/>
                            <InputField id="lastName" label="Nom" value={formData.lastName} onChange={onChange}
                                        disabled={loading} error={errors.lastName} placeholder="Votre nom"
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-1" aria-label="Boutons SSO">
                        <button type="button" disabled
                                className="btn bg-bg-muted text-muted-foreground cursor-not-allowed"
                                aria-disabled="true" title="Bientôt">Google
                        </button>
                        <button type="button" disabled
                                className="btn bg-bg-muted text-muted-foreground cursor-not-allowed"
                    ...
                    {emailNotVerified && mode === 'login' && (
                        <button type="button" onClick={resend} disabled={resent}
                            ...
                            l&apos;email de vérification</button>
                    )}
                </form>
