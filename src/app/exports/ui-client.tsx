...
export default function ExportAttachmentsClient({ properties }: { properties: PropertyOpt[] }) {
  ...
  async function onExport(e: React.FormEvent) {
    ...
    const q = new URLSearchParams({ propertyId, from: computed.from, to: computed.to });
    const url = `/api/exports/attachments.zip?${q.toString()}`;
    setLoading(true);
    const t = toast('Préparation de l’archive…', { description: `${computed.from} → ${computed.to}` });
    try {
    ...
    <form onSubmit={onExport} className="space-y-4">
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-4 text-sm">
            <label className="flex items-center gap-2"><input type="radio" name="mode" checked={mode==='month'} onChange={()=>setMode('month')} /> Mois</label>
            <label className="flex items-center gap-2"><input type="radio" name="mode" checked={mode==='range'} onChange={()=>setMode('range')} /> Intervalle</label>
          </div>
        </div>
      </div>
      ...
      {mode === 'month' ? (
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <div className="text-xs text-muted-foreground flex items-end">Export du mois sélectionné</div>
        </div>
      ) : (
      ...
      <div>
        <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Préparation…' : 'Exporter pièces (ZIP)'}</button>
      </div>
    </form>
  );
}
