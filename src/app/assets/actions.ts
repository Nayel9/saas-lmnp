...
const assetSchema = z.object({
  ...
  account_code: z.string().refine(v => assetAccountCodes.includes(v), 'Compte immobilisation inconnu')
});
...
export async function createAsset(formData: FormData) {
  ...
  if (!isAllowed(account_code,'asset')) return { ok: false, error: 'Compte immobilisation invalide' };
...
export async function updateAsset(formData: FormData) {
  ...
  if (!isAllowed(account_code,'asset')) return { ok: false, error: 'Compte immobilisation invalide' };
  const user_id = await getUserId();
  const { id, label, amount_ht, duration_years, acquisition_date } = parsed.data;
