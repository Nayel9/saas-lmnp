...
export default async function AssetAmortizationPage({
  params,
}: {
  ...
  const schedule = computeLinearAmortization(
    Number(asset.amount_ht),
    ...
    <main className="p-6 max-w-4xl mx-auto space-y-6">
      <header className="space-y-2">
        <h1 className="text-2xl font-semibold tracking-tight">
          Amortissement - {asset.label}
        </h1>
        <p className="text-sm text-muted-foreground">
          Montant HT: {formatAmount(Number(asset.amount_ht))} • Durée:{" "}
          {asset.duration_years} ans • Début:{" "}
          {asset.acquisition_date.toISOString().slice(0, 10)}
        </p>
        <div className="flex gap-2 text-sm">
          ...
          <Link
            href={`/api/assets/${asset.id}/amortization/export?format=csv`}
            className="btn"
          >
            Export CSV
          </Link>
            ...
            {schedule.map((r) => (
              <tr key={r.year} className="border-b last:border-none">
                ...
                <td className="py-1 pr-4 text-right tabular-nums">
                  {formatAmount(r.dotation)}
                </td>
                <td className="py-1 pr-4 text-right tabular-nums">
                  {formatAmount(r.cumul)}
                </td>
              </tr>
            ))}
          </tbody>
