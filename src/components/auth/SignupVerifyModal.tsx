...
export function SignupVerifyModal({
  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      try {
        const r = await fetch(
          `/api/auth/check-verified?email=${encodeURIComponent(email)}`,
          { cache: "no-store" },
        );
        if (r.ok) {
  ...
  useEffect(() => {
    if (opened) console.log("[SignupVerifyModal] opened for", email);
  }, [opened, email]);
  ...
  return (
    <>
      {opened && (
        <div
          data-testid="signup-verify-modal"
          role="dialog"
          aria-modal="true"
          aria-labelledby="signup-verify-title"
          aria-describedby="signup-verify-desc"
          onMouseDown={(e) => {
        ...
        >
          <div
            className={`bg-bg rounded-md shadow-xl w-full max-w-md p-6 flex flex-col gap-3 relative transform transition-all duration-300 ease-out will-change-transform will-change-opacity ${animate ? "opacity-100 scale-100" : "opacity-0 scale-90"}`}
          ...
          >
            ...
            <h2 id="signup-verify-title" className="text-lg font-semibold pr-8">
              Vérifiez vos emails
            </h2>
            <div
              id="signup-verify-desc"
              className="text-sm flex flex-col gap-2"
            >
              <span className="text-sm">
                Nous avons envoyé un lien de confirmation à{" "}
                <strong>{email}</strong>. Cliquez dessus pour activer votre
                compte.
              </span>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                <button
                  ...
                  type="button"
                  onClick={doResend}
                  disabled={inCooldown || resendState === "loading"}
              ...
              <span className="text-xs text-muted-foreground">
                Pas reçu ? Vérifiez les indésirables.
              </span>

              {message && (
                <div
                  data-testid="resend-alert"
                  className={`rounded p-2 mt-2 text-xs ${resendState === "error" ? "bg-red-100 text-red-800" : "bg-blue-100 text-blue-800"}`}
                >
                  <strong className="block mb-1">
                    {resendState === "error" ? "Erreur" : "Info"}
                  </strong>
                  {message}
                </div>
              )}
