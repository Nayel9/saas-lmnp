...
async function fetchListFor(parent: {
  entryId?: string;
  assetId?: string;
}): Promise<AttachmentDto[]> {
  if (parent.entryId) {
    const res = await fetch(`/api/entries/${parent.entryId}/attachments`, {
      cache: "no-store",
    });
  ...
  if (parent.assetId) {
    const res = await fetch(`/api/assets/${parent.assetId}/attachments`, {
      cache: "no-store",
    });
    if (!res.ok) throw new Error("Erreur liste");
    return res.json();
...
async function createAttachment(parent: {
  entryId?: string;
  assetId?: string;
  fileName: string;
  fileSize: number;
  ...
  const payload = entryId
    ? { entryId, fileName, fileSize, mimeType, storageKey }
    : { assetId, fileName, fileSize, mimeType, storageKey };
...
export function AttachmentsPanel({
  ...
  assetId?: string;
  onCloseAction: () => void;
  onCountChangeAction?: (n: number) => void;
}) {
  ...
  const modal = (
    ...
    >
      <div
        className="bg-card rounded-md shadow-md w-full max-w-lg p-5 space-y-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-medium flex items-center gap-2">
            Pièces jointes {busy && <Spinner />}
          </h2>
          <button className="btn" onClick={onCloseAction}>
            Fermer
          </button>
        </div>
        <div
          ...
          onDrop={onDrop}
          className="border border-dashed rounded-md p-4 text-sm text-muted-foreground"
        >
          ...
          <div className="mt-2">
            <button
              ...
              disabled={busy}
            >
              {busy && <Spinner />}Choisir un fichier
            </button>
            <input
              ref={fileInputRef}
              type="file"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b">
                  ...
                  <th className="py-1 pr-2">Taille</th>
...
export function AttachmentsPreviewTrigger({
  entryId,
  assetId,
}: {
  entryId?: string;
    ...
    <span className="relative inline-block ml-2">
      <button
        ...
        onClick={(e) => {
          ...
          setModalOpen(true);
          setLoading(true);
          Promise.resolve().then(ensureLoaded);
        }}
            ...
            <div className="bg-card border rounded-md p-2 shadow-lg max-w-[90vw] max-h-[90vh] flex items-center justify-center relative">
              ...
              {!loading && previewUrl && (
                ...
                <img
                  ...
                  alt="Aperçu"
                  style={{
                    maxWidth: `${maxSize.maxWidth}px`,
                    maxHeight: `${maxSize.maxHeight}px`,
                    width: "auto",
                    height: "auto",
                  }}
