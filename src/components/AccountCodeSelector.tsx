...
import {
  ...
  searchAccounts,
  type AccountContext,
} from "@/lib/accounting/accountsCatalog";

interface Props {
  typeJournal: AccountContext;
  name?: string;
  defaultValue?: string;
  onChange?: (v: string) => void;
  onInvalidSelection?: (typed: string) => void;
  required?: boolean;
  allowFree?: boolean;
}
...
export function AccountCodeSelector({
  ...
  const onKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Enter") {
        if (open && displayList[highlightIndex]) {
          e.preventDefault();
      ...
      if (e.key === "ArrowDown") {
        e.preventDefault();
        setHighlightIndex((i) => Math.min(displayList.length - 1, i + 1));
  ...
  useEffect(() => {
    if (highlightIndex > displayList.length - 1) setHighlightIndex(0);
  }, [displayList, highlightIndex]);
      ...
      <div className="relative">
        <div className="flex gap-2">
          ...
          <div className="flex items-center text-[10px] gap-1">
            {allowed && value && (
              <span className="badge text-brand" aria-label="Compte valide">
                {value}
              </span>
            )}
            {!value && (
              <span
                className="badge text-muted-foreground"
        ...
        {open && (
          <div
            className="absolute z-50 mt-1 max-h-64 overflow-auto w-full rounded-md border bg-card shadow"
            role="listbox"
          >
