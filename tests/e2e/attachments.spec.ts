...
function minimalJpg(): Buffer {
  return Buffer.from([0xff, 0xd8, 0xff, 0xd9]);
}
function minimalTxt(): Buffer {
...
test.describe.serial("Pièces jointes – E2E", () => {
  ...
  test("3) Erreur type interdit .txt", async ({ page }) => {
    ...
    await expect(
      page.getByText(/Upload échoué|Pré-signature échouée/),
    ).toBeVisible();
  });

  test("4) Protection multi-utilisateurs (download interdit)", async ({
    browser,
  }) => {
    const { email, password } = await ensureAdminCreds();
