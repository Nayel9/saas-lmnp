...
test("Flux inscription / vérification / login avec affichage prénom", async ({
  page,
  request,
}) => {
  ...
  await expect(
    page.getByRole("dialog", { name: /Vérifiez vos emails/i }),
  ).toBeVisible();
  await expect(page.getByText(email)).toBeVisible();

  // Récupération du token de vérification via endpoint debug
  const debugResp = await request.get(
  ...
  // Vérification
  await page.goto(
    `/auth/verify?token=${encodeURIComponent(data.token)}&email=${encodeURIComponent(email)}`,
  );
  await page.waitForURL(/login\?verified=1/);
  await expect(
    page.getByText("Email vérifié, vous pouvez maintenant vous connecter."),
  ).toBeVisible();
  ...
  await expect(
    page.getByRole("navigation").getByText("Claire Durand", { exact: true }),
  ).toBeVisible();
});
