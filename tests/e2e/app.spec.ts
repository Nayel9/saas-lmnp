...
async function uiLogout(page: Page) {
  ...
  await page.waitForURL(
    (url) => /\/login$/.test(url.pathname) || /\/$/.test(url.pathname),
  );
}

const UNIQUE_RUN_ID = Date.now();
function uniq(label: string) {
  return `${label}-${UNIQUE_RUN_ID}`;
}
...
test.describe.serial("E2E scénario complet", () => {
  ...
  test("2) Journaux achats & ventes CRUD + exports", async ({ page }) => {
    ...
    await page
      .locator(`tr:has(td:has-text("${achatInitial}")) button:has-text("Edit")`)
      .click();
    await page.fill('input[name="designation"]', achatEdited);
    await page.click('button:has-text("Mettre à jour")');
    await expect(
      page.locator(`td:has-text("${achatEdited}")`).first(),
    ).toBeVisible();
  ...
  test("3) Immobilisation + amortissement + exports", async ({ page }) => {
    ...
    const accountSearch = page.locator(
    ...
    const accountSearch2 = page.locator(
      'input[aria-label="Recherche compte comptable"]',
    );
    ...
    await expect(page.getByTestId("asset-form-error")).toContainText(
    ...
    await expect(
      page.locator(`h1:has-text("Amortissement - ${assetLabel}")`),
    ).toBeVisible();
    ...
    const dotationCellText = (
      await row2024.locator("td").nth(1).innerText()
    ).trim();
    ...
    const xlsxResp = await retry(() =>
      page.request.get(`/api/assets/${id}/amortization/export?format=xlsx`),
    );
  ...
  test("4) Rapports Balance & Grand Livre", async ({ page }) => {
    ...
    if (rowCount > 0) {
      const footerDebit = parseMoney(
        await page.locator("tfoot td").nth(1).innerText(),
      );
      const footerCredit = parseMoney(
        await page.locator("tfoot td").nth(2).innerText(),
      );
      const footerBalance = parseMoney(
        await page.locator("tfoot td").nth(3).innerText(),
      );
      expect(Number((footerDebit - footerCredit).toFixed(2))).toBe(
        Number(footerBalance.toFixed(2)),
      );
    }
    ...
    if (accText) {
      await page.click(
        `a[href*="/reports/ledger?account_code=${accText.trim()}"]`,
      );
      await expect(page.locator('h1:has-text("Grand livre")')).toBeVisible();
      const ledgerCsv = await Promise.all([
        page.waitForEvent("download"),
        page.click('a:has-text("Export CSV")'),
      ]);
      expect(await ledgerCsv[0].path()).toBeTruthy();
      saveDownloadCopy(
        await ledgerCsv[0].path(),
        `ledger-${UNIQUE_RUN_ID}.csv`,
      );
    }

    await page.goto("/reports/2033c");
    await expect(
      page.locator('h1:has-text("Compte de résultat 2033-C")'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('h1:has-text("État des amortissements 2033-E")'),
    ).toBeVisible();
    ...
    await expect(
      page.locator('h1:has-text("Bilan simplifié 2033-A")'),
    ).toBeVisible();
  ...
  test("5) Isolation second utilisateur", async ({ browser }) => {
    ...
    await page.goto("/journal/achats");
    await expect(
      page.locator(`td:has-text("${achatEditedGlobal || ""}")`),
    ).toHaveCount(0);
    await context.close();
  });
});
...
function saveBuffer(buf: Buffer, targetBase: string) {
  const dest = path.join(ARTIFACT_DIR, targetBase);
  try {
