...
function minimalPng(): Buffer { return Buffer.from([0x89,0x50,0x4E,0x47,0x0D,0x0A,0x1A,0x0A]); }
...
async function attachToRow(page: Page, rowLabel: string, filePath: string) {
  ...
  await expect(page.locator(`td:has-text("${path.basename(filePath)}")`)).toBeVisible();
...
test.describe.serial('Exports pièces (ZIP) – E2E', () => {
  ...
  test('2) Sécurité: autre utilisateur ne peut pas exporter le bien d’un autre', async ({ browser }) => {
    const { email, password } = await ensureAdminCreds();
    ...
    const prop = await prisma.property.create({ data: { user_id: owner!.id, label: 'Bien Privé', address: null } });
    ...
    const from = new Date();
    const to = new Date();
    const qs = new URLSearchParams({ propertyId: prop.id, from: toISO(from), to: toISO(to) });
    const res = await pageB.request.get('/api/exports/attachments.zip?' + qs.toString());
  ...
  test('3) Période sans pièces: ZIP avec seulement index.csv', async ({ page }) => {
    const { email, password } = await ensureAdminCreds();
    await uiLogin(page, email, password);

    const owner = await prisma.user.findUnique({ where: { email } });
    const props = await prisma.property.findMany({ where: { user_id: owner!.id }, take: 1 });
    let propId = props[0]?.id;
    if (!propId) {
      const created = await prisma.property.create({ data: { user_id: owner!.id, label: 'Bien Vide', address: null } });
      propId = created.id;
    }

    // période lointaine sans pièces
    const qs = new URLSearchParams({ propertyId: propId!, from: '2000-01-01', to: '2000-01-31' });
    const dl = await page.request.get('/api/exports/attachments.zip?' + qs.toString());
