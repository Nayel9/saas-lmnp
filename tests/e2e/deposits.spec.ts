...
test.describe.skip("Cautions (ventes)", () => {
  test("création, badge, totaux exclus, dashboard et synthèse", async ({
    page,
    browser,
  }) => {
    ...
    await expect(
      page.locator('td:has-text("Vente normale 500")'),
    ).toBeVisible();
    ...
    await expect(
      page.locator(
        'tr:has(td:has-text("Caution 200")) .badge:has-text("Caution")',
      ),
    ).toBeVisible();
    ...
    await expect(
      page.locator('td:has-text("Cautions détenues") ~ td'),
    ).toContainText(/200|200,00|200\.00/);
