...
const url = process.env.FORCE_DIRECT
  ? process.env.DIRECT_URL
  : process.env.DATABASE_URL;
if (!url) {
  console.error(
    "URL manquante (" +
      (process.env.FORCE_DIRECT ? "DIRECT_URL" : "DATABASE_URL") +
      ")",
  );
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    if (e.message.includes("certificate"))
      console.error("→ SSL: ajouter sslmode=require");
  } finally {
